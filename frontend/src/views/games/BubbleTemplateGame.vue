  ...
  <div class="page-field">
    <section class="template-stage">
      ...
      <div class="stage-copy content-layer">
        <p class="section-kicker">Template Game 2</p>
        <h1 class="section-title">{{ game.name }}</h1>
        <p class="section-copy">模板二用于演示另一种页面装扮。以后不同游戏可以有不同视觉，但必须保留入口、规则、本体、记录、后端统计这条链。</p>

        <div class="stage-actions">
          <button class="park-button" @click="startRound">开始模板二</button>
        ...
        <div class="structure-note">
          复制新增游戏时，也可以参考这个文件：{{ game.pageFile }}。
        </div>
      </div>
      ...
      <div class="stage-board content-layer" :class="{ active: playing, done }">
        ...
        <span>{{ boardHint }}</span>
        <button v-if="playing" class="park-button ghost" @click="finishRound">点亮泡泡</button>
      </div>
    </section>
  </div>
</template>
...
const game = findGame('bubble-template')
const playing = ref(false)
const done = ref(false)

const boardTitle = computed(() => {
  if (done.value) return '模板二完成'
  if (playing.value) return '泡泡亮起来了'
  return '等你开始'
})

const boardHint = computed(() => {
  if (done.value) return '模板二已经完成记录链路。'
  if (playing.value) return '点击泡泡，写入一条模板记录。'
  return '按下开始模板二。'
})
